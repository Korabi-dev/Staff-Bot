module.exports = {
    run: async(client, message, args) => {
        ...
        if(user.roles.cache.get(client.staffrole)){
            user.roles.remove(client.staffrole)
        }
        if(doc){
            await client.models.staff.findOneAndDelete({user: user.id})
            client.log(client.embed("Staff Removed!", `Everyone say goodbye to \`${user.user.username}\`, they are no longer on the staff team!`))
        }
        message.reply(client.embed("Success", `You have removed ${user} from the staff team.`))
    }
}